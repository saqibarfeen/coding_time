...
#
barplot(prevs,space=0.5,names.arg=NULL,horiz=TRUE
,axes=FALSE,xlim=c(0,50),xlab="Percent",col=0)
#
yy<-1.5*(1:length(prevs)-0.33)
#
for (i in 1:6) {
lines(x=c(prevs[i]-se[i],prevs[i]+se[i]),y=rep(yy[i],2))
lines(x=rep(prevs[i]-se[i],2),y=c(yy[i]+0.33/2,yy[i]-0.33/2))
lines(x=rep(prevs[i]+se[i],2),y=c(yy[i]+0.33/2,yy[i]-0.33/2))
}
#
axis(1,tick=TRUE,xaxp=c(0,50,5))
yy<-1.5*(1:length(prevs)-.5)
axis(2,at=yy+0.1,labels=names,font=1,
tck=0,tcl=0,las=1,padj=0,col=0)
#
dev.copy2eps(file="fig03x011.eps")
dev.copy2pdf(file="fig03x011.pdf")
}
