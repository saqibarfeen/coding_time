...
stem(mass,scale=2)
sink()
zz<-readLines("fig04x008.txt",warn=TRUE)
zz<-paste(zz," ")
text(rep(0,length(zz)),sort(1:length(zz),
decreasing=TRUE)/(length(zz)),zz,pos=4,offset=0,font=1)
#
dev.copy2eps(file="fig04x008.eps",fonts="mono",family="mono")
dev.copy2pdf(file="fig04x008.pdf")
}
