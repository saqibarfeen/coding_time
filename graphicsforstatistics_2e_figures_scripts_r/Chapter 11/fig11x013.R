...
summolm<-summary(molm)
rsemistand<-residuals(molm)/summolm$sigma
#
graphics.off()
...
#
qqnorm(rsemistand,datax=TRUE,main=" ",cex=0.5,
axes=FALSE,ylim=c(-4,4),xlab=" ",ylab=" ")
#
title(ylab="Standard Normal Quantiles",
xlab="Semistandardized Residual")
#
axis(1,-4:4)
axis(2,-3:3)
box("plot")
#
qqline(rsemistand,datax=TRUE)
#
par(mai=c(0.0,0.85,0.0,0.25))
#
qqnorm(rsemistand,datax=TRUE,main=" ",pch=" ",
axes=FALSE,ylab=" ",xlab=" ",ylim=c(-4,4))
#
boxplot(rsemistand,horizontal=TRUE,
add=TRUE,axes=FALSE,boxwex=6,cex=0.5)
#
dev.copy2eps(file="fig11x013.eps")
dev.copy2pdf(file="fig11x013.pdf")
}
