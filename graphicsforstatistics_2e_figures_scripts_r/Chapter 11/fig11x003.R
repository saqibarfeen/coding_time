...
for (i in 1:23) {
         for (j in 1:18) {
mip<-c(mip,rep(j,ParOff[i,j]))
off<-c(off,rep(i,ParOff[i,j]))
}
}
#
mip<- mip + 60.5
...
set.seed(345)
#
sunflowerplot(mip,off,
xlab="Mid-Parent Height (inch)",
ylab="Offspring Height (inch)",xlim=c(62.5,77.5)
,ylim=c(55,80),pch=19,seg.col="black",cex=0.8,cex.fact=3,
...
plot.points=FALSE)
#
dev.copy2eps(file="fig11x003.eps")
dev.copy2pdf(file="fig11x003.pdf")
}
