...
FUN=sum)
se<-sdf/sqrt(counts)
sds<-sdf/sqrt(2*counts)
#
qf<-qnorm(0.975)
#
stripchart(fc$Weight ~ fc$Species,col="white",
xlim=c(0,1000),xlab="Mass (g)",
group.names=rep(" ",7),vertical=FALSE)
#
for (i in 1:7) points(means[i],i,pch=19)
for (i in 1:7) lines(c(means[i]-qf*sds[i],means[i]+qf*sds[i]),c(i,i))
for (i in 1:7) lines(rep(means[i]-se[i],2),c(i-0.125,i+0.125))
for (i in 1:7) lines(rep(means[i]+se[i],2),c(i-0.125,i+0.125))
#
axis(2,at=1:7,labels=levels(fc$Species),las=1)
#
dev.copy2eps(file="fig09x013.eps")
dev.copy2pdf(file="fig09x013.pdf")
}
